...
class BasicBlock(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.conv1 = conv_factory(in_planes, planes, kernel_size=3, stride=stride, padding=1, bias=False)
        self.bn1 = nn.BatchNorm2d(planes)
        self.conv2 = conv_factory(planes, planes, kernel_size=3, stride=1, padding=1, bias=False)
        self.bn2 = nn.BatchNorm2d(planes)

        self.shortcut = nn.Identity()
        if stride != 1 or in_planes != self.expansion * planes:
            self.shortcut = nn.Sequential(
                conv_factory(in_planes, self.expansion * planes, kernel_size=1, stride=stride, padding=0, bias=False),
...
class BitResNet(Module):
    ...
    def __init__(
        ...
        self.conv1 = self.conv_factory(in_channels, base_channels, kernel_size=3, stride=1, padding=1, bias=False)
