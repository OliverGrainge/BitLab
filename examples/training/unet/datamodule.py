"""PyTorch Lightning datamodule for the LSUN Bedrooms dataset."""
...
class LSUNBedroomsDataModule(pl.LightningDataModule):
    """Lightning datamodule for the Hugging Face `pcuenq/lsun-bedrooms` dataset."""
    ...
    def _transform_batch(self, examples: Dict[str, Any]) -> Dict[str, Any]:
        images = [self.image_transform(image) for image in examples["image"]]
    ...
    def _collate_fn(self, batch: Any) -> torch.Tensor:
        ...
        raise TypeError("Unsupported batch format for LSUNBedroomsDataModule")
