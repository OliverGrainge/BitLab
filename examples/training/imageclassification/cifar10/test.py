"""
Evaluate a trained CIFAR-10 classifier checkpoint.
...
def build_datamodule(config: Dict[str, Any], *, batch_size_override: int | None = None) -> CIFAR10ClassificationDataModule:
...
def load_checkpoint(lit_module: BitImageClassifierTrainer, checkpoint_path: Path, device: torch.device) -> None:
    checkpoint = torch.load(checkpoint_path, map_location=device)
    if "state_dict" not in checkpoint:
        raise KeyError(f"Checkpoint at '{checkpoint_path}' does not contain a 'state_dict' entry.")

    missing, unexpected = lit_module.load_state_dict(checkpoint["state_dict"], strict=False)
    if missing:
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate a trained CIFAR-10 classifier checkpoint.")
    parser.add_argument(
        "config",
        type=Path,
        nargs="+",
...
def main(argv: list[str]) -> int:
    ...
    exit_code = 0
    for config_path, checkpoint_override in zip(args.config, checkpoints, strict=False):
        try:
